...
describe("request URL resolution", () => {
  ...
  it("posts memory source sync to the gateway's production route", async () => {
    const fetchMock = vi.fn().mockImplementation(async () =>
      new Response(
        JSON.stringify({ items: [], synced: 0, failed: 0 }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }
    ...
    expect(url).toBe("http://127.0.0.1:18888/api/v1/memory/sync");
