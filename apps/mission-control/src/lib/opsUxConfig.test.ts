...
describe("opsUxRuntimeConfig", () => {
  ...
  it("sanitizes malformed runtime config payloads", () => {
    const sanitized = sanitizeOpsUxRuntimeConfig({
      ...
      safety: {
        recovery_retention_window_ms: 45 * 60_000,
        mark_read_undo_window_ms: -4,
      },
    });
    ...
    expect(sanitized.safety.recovery_retention_window_ms).toBe(45 * 60_000);
    expect(sanitized.safety.mark_read_undo_window_ms).toBe(
