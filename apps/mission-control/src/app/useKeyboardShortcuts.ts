  ...
  const handler = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (!overlayOpen && !isEditableTarget(e) && !meta && !e.altKey && !e.shiftKey) {
        ...
        const roomIsAvailable = (room: { route: MissionControlTab }) =>
          elevatorFloors ? true : availableTabs.includes(room.route);
        if (defaultRoom && roomIsAvailable(defaultRoom)) {
          jumpTo(defaultRoom);
          return;
        }
        const fallbackRoom = floor?.rooms.find(roomIsAvailable);
        if (fallbackRoom) {
          jumpTo(fallbackRoom);
        }
      }
