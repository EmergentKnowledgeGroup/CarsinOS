...
describe("useResolvedElevator", () => {
  ...
  it("keeps an explicitly independent room when its shared route is unavailable", async () => {
    function Harness() {
      const floors = useResolvedElevator(["boards"], ["setup"]);
      return (
        <output>
          {floors.flatMap((floor) => floor.rooms.map((room) => room.id)).join(",")}
        </output>
    ...
    expect(container.textContent).toBe("boards,setup");
    await act(async () => root.unmount());
  });
});
