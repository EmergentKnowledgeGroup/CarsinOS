...
import {
  FocusPage,
  type FocusQueueIntent,
} from "../features/focus/FocusPage";
import { TeamPage } from "../features/team/TeamPage";
import type { PeopleRoutingController } from "../features/peopleRouting/usePeopleRoutingController";
import { HelpDocsPage } from "../features/help/HelpDocsPage";
import {
  MemoryPage,
  type MemoryLandingIntent,
} from "../features/memory/MemoryPage";
...
interface AppContentProps {
   ...
   * Returns whether the resolved elevator accepted the room.
   */
  onRoomSelect: (roomId: string) => boolean;
  /** Whether the resolved elevator currently exposes the Memory plant room. */
  memoryRoomAvailable: boolean;
  /** Resolved stable room id owning the lamp; shared-route surfaces land by it. */
...
export function AppContent(props: AppContentProps) {
  ...
  // Memory plant landing seam. Entering the stable `memory` room lands on the
  // global plant; a Staff-card entry names the exact agent instead. Only the
  // newest explicit room intent re-lands the page, so internal Memory choices
  // survive unrelated rerenders and tab switches. `entryHandled` marks an
  // entry whose intent was already set by its event handler, so the room
  // derivation below does not overwrite it with a plant landing.
  const [memoryLanding, setMemoryLanding] = useState<
    MemoryLandingIntent & { entryHandled: boolean }
  >({
    seq: 0,
    kind: "plant",
    agentId: null,
  ...
  // Room-entry detection is a pure local-state derivation (the TeamPage
  // lastRoomId pattern), not an effect.
  const [lastActiveTab, setLastActiveTab] = useState(active);
  if (active !== lastActiveTab) {
    setLastActiveTab(active);
    if (active === "memory") {
      setMemoryLanding((prev) =>
        prev.entryHandled
          ? { ...prev, entryHandled: false }
          : { seq: prev.seq + 1, kind: "plant", agentId: null, entryHandled: false }
      );
    }
  }
  const e2eMode =
  ...
  const [focusQueueIntent, setFocusQueueIntent] = useState<FocusQueueIntent>({
    nonce: 0,
    targetKind: null,
    targetId: null,
  });
  ...
  const openAgentMemory =
    props.memoryController.enabled && props.memoryRoomAvailable
    ? (agentId: string) => {
        if (!props.onRoomSelect("memory")) {
          props.setNotice({
            tone: "error",
            message: "The Memory plant room is not available right now.",
          });
          return;
        }
        // The exact requested agent wins, but an already-open drill-in for
        // that agent is not torn down just to reland on it.
        if (props.memoryController.selectedAgentId !== agentId) {
          props.memoryController.setSelectedAgentId(agentId);
        }
        setMemoryLanding((prev) => ({
          seq: prev.seq + 1,
          kind: "agent",
  ...
  const openRunbookDeepLink = (target: {
    ...
    if (target.tab === "focus") {
      // Carry the exact queue entity through the room landing. Merely opening
      // Queue can strand the requested approval or job on another page.
      setFocusQueueIntent((current) => ({
    ...
    <>
      ...
      >
        ...
        <FocusPage
          ...
          jobsStatus={props.missionControl.jobsStatus}
          openBreakers={props.missionControl.openBreakers}
          openPluginBreakers={props.missionControl.openPluginBreakers}
          calendarJobs={props.missionControl.calendarJobs}
          onToggleCalendarJob={props.missionControl.toggleCalendarJob}
      ...
      >
        ...
        <MailPage
          agents={props.agents}
          activeRoomId={props.activeRoomId}
          peopleRouting={props.peopleRoutingController}
          ...
          leasesLoading={props.mailController.leasesLoading}
          leasesLoaded={props.mailController.leasesLoaded}
          leasesError={props.mailController.leasesError}
          onRetryLeases={() => props.mailController.refreshLeases()}
      ...
      >
        ...
        <TeamPage
          ...
          peopleRouting={props.peopleRoutingController}
          onOpenPeopleRouting={() => props.onRoomSelect("directory")}
          onOpenAgentMemory={openAgentMemory}
        />
      </TabBoundaryPane>
