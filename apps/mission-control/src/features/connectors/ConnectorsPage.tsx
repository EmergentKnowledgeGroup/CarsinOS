...
interface ConnectorsPageProps {
  ...
  /**
   * The resolved stable room id from the elevator. Two Basement rooms share
   * this route: "connectors" must land on connector management and "setup"
   * on the distinct Setup room surface. Any other value leaves the
   * surface's own tab state alone.
   */
  activeRoomId: string | null;
  /**
   * The one shared Setup authority (connection drafts/actions, feature
   * controls, onboarding/tour launchers) App also hands to Settings. The
...
/**
 * Landing surface per stable room id. The Setup room renders its own
 * distinct surface and must never touch the Connectors tab truth, so only
 * the Connectors room appears here; no route-name or display-label guesses.
 */
const ROOM_LANDING_TAB: Readonly<Partial<Record<string, ConnectorsTab>>> = {
  connectors: "registry",
};
...
export function ConnectorsPage({
  ...
  // Only an actual room change may force a landing surface; internal tab
  ...
  if (activeRoomId !== lastRoomId) {
    ...
    const landing = activeRoomId ? ROOM_LANDING_TAB[activeRoomId] : undefined;
    if (landing) {
  ...
  // Setup is the independent authority that can turn Connectors back on.
  // It shares this mounted route only to preserve the existing pane and
  // controller lifecycle; connector availability must never gate the room.
  if (activeRoomId === "setup") {
  ...
  return (
    <section className="mc-connectors-page mc-connectors-paged" data-testid="connectors-page">
      <div className="mc-room-tabs-row">
        ...
        {/* The pin belongs to the Connectors room identity, not the shared
            surface: the Setup room renders its own pin on its own surface,
            and one door must never ambiguously open both rooms. */}
        {activeRoomId === "connectors" ? (
          <PinRoomToOffice roomId="connectors" />
        ) : null}
      </div>
