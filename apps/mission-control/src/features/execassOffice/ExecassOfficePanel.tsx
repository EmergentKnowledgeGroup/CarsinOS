...
function RoomShortcutBody(props: {
  ...
  // Refusal feedback is remembered against the exact resolved room-select
  // callback that produced it: the callback changes identity precisely when
  // the elevator registry changes (tab availability or floor overrides), and
  // ops-config toggles like the Runbook page never fire GLASS_CONFIG_EVENT —
  // so a re-enabled room may not keep a stale unavailable door label.
  const [refusedBy, setRefusedBy] = useState<
    ((roomId: string) => boolean | void) | null
  >(null);
  useEffect(() => {
    const clearStaleAvailability = () => setRefusedBy(null);
  ...
  const unavailable = refusedBy === props.onOpenRoom;
  ...
  return (
    <div className="mc-room-shortcut">
      ...
      <button
        ...
        onClick={() => {
          const refused = props.onOpenRoom(found.room.id) === false;
          setRefusedBy(refused ? () => props.onOpenRoom : null);
        }}
      >
        {unavailable ? (
...
export function ExecassOfficePanel(props: {
  ...
  // ≤760px matches the stylesheet's stacked-feed breakpoint. The mobile
  // presentation leads with Needs You without touching the stored
  // arrangement; arrange mode always shows the true stored order.
  const narrowViewport = useNarrowViewport(760);
              ...
              <>
                ...
                >
                  {(() => {
                    ...
                    if (!narrowViewport || layout.arranging) {
                      return visiblePlacements;
                    }
                    return [...visiblePlacements].sort(
                      (a, b) =>
                        Number(b.id === "needs-you") -
                        Number(a.id === "needs-you"),
                    );
                  })().map((placement) => {
