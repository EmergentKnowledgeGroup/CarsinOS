...
describe("PinRoomToOffice", () => {
  ...
  it("offers the Event stream pin under its room label", async () => {
    await render("events");
    const button = pinButton();
    expect(button).toBeTruthy();
    expect(button?.getAttribute("aria-label")).toBe(
      "Pin Event stream to Office",
    );
  });
  ...
  it("offers the Connectors pin under its room label", async () => {
    ...
    expect(button).toBeTruthy();
    expect(button?.getAttribute("aria-label")).toBe(
      "Pin Connectors to Office",
    );
  });

  it("offers the History & Receipts pin under its room label", async () => {
    await render("history");
    const button = pinButton();
    expect(button).toBeTruthy();
    expect(button?.getAttribute("aria-label")).toBe(
      "Pin History & Receipts to Office",
    );
  });

  it("offers the Staff Directory pin under its room label", async () => {
    await render("staff");
    const button = pinButton();
    expect(button).toBeTruthy();
    expect(button?.getAttribute("aria-label")).toBe(
      "Pin Staff Directory to Office",
    );
  });
