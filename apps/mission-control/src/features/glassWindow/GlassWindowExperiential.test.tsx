...
describe("Narrow Window summaries", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });
  ...
  it("summarizes Chatter before disclosure at narrow widths", async () => {
    stubNarrow();
    await mount(makeController());
    const collapse = container.querySelector<HTMLDetailsElement>(
      "details.mc-chatter-collapse",
    );
    expect(collapse).not.toBeNull();
    expect(collapse?.open).toBe(false);
    const summary = collapse?.querySelector("summary")?.textContent ?? "";
    expect(summary).toContain("2 rooms");
    expect(summary).toContain("1 with unread notes");
    expect(
      container.querySelector("details.mc-reef-collapse"),
    ).not.toBeNull();
  });

  it("keeps the full Chatter layout direct on desktop widths", async () => {
    await mount(makeController());
    expect(container.querySelector("details.mc-chatter-collapse")).toBeNull();
    expect(container.querySelector(".mc-chatter-layout")).not.toBeNull();
  });
});
