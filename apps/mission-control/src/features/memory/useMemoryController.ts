...
/**
 * Memory's window into the one People & Routing write authority (PR #113).
 * Lane policy edits must flow through this shared controller; Memory may
 * read routing facts for posture but never saves a full routing object of
 * its own over Front Desk.
 */
type SharedRoutingAuthority = Pick<PeopleRoutingController, "routingConfig"> & {
  saveLaneMemoryPolicy: NonNullable<PeopleRoutingController["saveLaneMemoryPolicy"]>;
};
...
interface UseMemoryControllerOptions {
  ...
  peopleRouting: SharedRoutingAuthority;
  setNotice: NotifyFn;
}
...
export function useMemoryController(options: UseMemoryControllerOptions) {
  ...
  const {
    saveLaneMemoryPolicy: saveSharedLaneMemoryPolicy,
    routingConfig: sharedRoutingConfig,
  } = peopleRouting;
  ...
  const [laneStatusAgentId, setLaneStatusAgentId] = useState("");
  ...
  // React render state cannot reject two same-tick submissions; each mutation
  // owns a synchronous lock. The two source syncs share one lock because they
  // are sibling mutations against the same sync endpoint.
  const runtimeSaveLockRef = useRef(false);
  const laneSaveLockRef = useRef(false);
  const sourceSyncLockRef = useRef(false);
  ...
  // Agent selection can change indirectly (a preferred agent arrives or the
  // roster changes) without going through setSelectedAgentId. Scope every
  // posture fact at render time so old status/lane data cannot flash under the
  // newly derived assistant before its own requests finish.
  const selectedStatus =
    status?.agent_id === selectedAgentId ? status : null;
  const selectedLaneStatuses =
    laneStatusAgentId === selectedAgentId ? laneStatuses : [];
  const selectedCardsResponse =
    cardsResponse?.agent_id === selectedAgentId ? cardsResponse : null;
  const selectedEpisodesResponse =
    episodesResponse?.agent_id === selectedAgentId ? episodesResponse : null;
  const selectedGraphMapResponse =
    graphMapResponse?.agent_id === selectedAgentId ? graphMapResponse : null;
  const selectedRuntimeHealthResponse =
    runtimeHealthResponse?.agent_id === selectedAgentId
      ? runtimeHealthResponse
      : null;
  const selectedTelemetrySummaryResponse =
    telemetrySummaryResponse?.agent_id === selectedAgentId
      ? telemetrySummaryResponse
      : null;
  const selectedTelemetryTurnsResponse =
    telemetryTurnsResponse?.agent_id === selectedAgentId
      ? telemetryTurnsResponse
      : null;
  const selectedDecisionReasonsResponse =
    decisionReasonsResponse?.agent_id === selectedAgentId
      ? decisionReasonsResponse
      : null;
  const bindingKey = useMemo(
    () => getMemoryBindingCacheKey(selectedAgentId, selectedStatus),
    [selectedAgentId, selectedStatus]
  );
  const nativeSurfaceAvailability =
    selectedStatus?.native_surface_availability ?? EMPTY_SURFACE_AVAILABILITY;
  const canRead = canLoadMemoryReadSurfaces(selectedStatus);

  // Visible lists must belong to the CURRENT agent. When the derived agent
  // selection flips (a late preferred-agent arrival, an agents-list change),
  // response state from the previous agent survives one render; deriving
  // card/atom/turn/citation targets from it would fire cross-agent detail
  // requests. Gating on the payload's own agent identity closes that seam.
  const cards = useMemo(
    () =>
      selectedCardsResponse
        ? selectedCardsResponse.data.cards
        : [],
    [selectedCardsResponse]
  );
  const episodes = useMemo(
    () =>
      selectedEpisodesResponse
        ? selectedEpisodesResponse.data.episodes
        : [],
    [selectedEpisodesResponse]
  );
  const graphNodes = useMemo(
    () =>
      selectedGraphMapResponse
        ? selectedGraphMapResponse.data.nodes
        : [],
    [selectedGraphMapResponse]
  );
  const graphLinks = useMemo(
    () =>
      selectedGraphMapResponse
        ? selectedGraphMapResponse.data.links
        : [],
    [selectedGraphMapResponse]
  );
  const telemetryTurns = useMemo(
    () =>
      selectedTelemetryTurnsResponse
        ? selectedTelemetryTurnsResponse.data.turns
        : [],
    [selectedTelemetryTurnsResponse]
  );
  const telemetrySummary = useMemo(
    () =>
      selectedTelemetrySummaryResponse
        ? selectedTelemetrySummaryResponse.data.summary
        : [],
    [selectedTelemetrySummaryResponse]
  );
  const decisionReasons = useMemo(
    () =>
      selectedDecisionReasonsResponse
        ? selectedDecisionReasonsResponse.data.reasons ?? []
        : [],
    [selectedDecisionReasonsResponse]
  );
  ...
  const loadMemoryData = useCallback(
    async (
      ...
      // A configured gateway URL is not configured authentication. Wait for
      // the real token-ready boundary and invalidate anything already in
      // flight when that boundary changes.
      if (!tokenConfigured || !runtimeSettings.gateway_url.trim()) {
        invalidateInFlightRequests();
        resetVisibleMemoryState("loading", "Waiting for the gateway connection.");
        return createSuccessResult();
      }
  ...
  const loadRoutingSnapshot = useCallback(
    async (runtimeSettings: RuntimeConnectionSettings = settings) => {
      ...
      if (!enabled || !tokenConfigured || !runtimeSettings.gateway_url.trim()) {
    ...
    [enabled, settings, tokenConfigured]
  );
  ...
  const loadLaneStatuses = useCallback(
    async (
      ...
      if (
        ...
        setLaneStatuses([]);
        setLaneStatusAgentId("");
        setLaneStatusError(null);
        setLaneStatusLoading(false);
        return createSuccessResult();
      }

      setLaneStatusAgentId(targetAgentId);
      setLaneStatuses([]);
    ...
    [enabled, selectedAgentId, settings, tokenConfigured]
  );
  ...
  const saveLaneMemoryPolicy = useCallback(
    async (
      ...
      laneSaveLockRef.current = true;
      setLanePolicySaveKey(pairKey);
      try {
        const savedRouting = await saveSharedLaneMemoryPolicy(
          humanIdentityId,
          assistantAgentId,
          memoryMode,
          options
        ...
        laneSaveLockRef.current = false;
        setLanePolicySaveKey(null);
      }
    },
    [
      loadLaneStatuses,
      saveSharedLaneMemoryPolicy,
      setNotice,
      settings,
      tokenConfigured,
    ]
  ...
  const saveRuntimeMemoryDefaults = useCallback(
    async (blendMode: string, memoryMdSources: string[]) => {
      ...
      runtimeSaveLockRef.current = true;
      ...
      try {
        ...
        runtimeSaveLockRef.current = false;
        setRuntimeMemorySavePending(false);
      }
    },
    [loadLaneStatuses, selectedAgentId, setNotice, settings, tokenConfigured]
  );

  const syncRuntimeMemoryDefaults = useCallback(async () => {
    if (sourceSyncLockRef.current) {
      return false;
    }
    if (!tokenConfigured || !settings.gateway_url.trim()) {
      setNotice({
    ...
    sourceSyncLockRef.current = true;
    ...
    try {
      ...
      sourceSyncLockRef.current = false;
  ...
  // `sharedRoutingConfig` is a real dependency: when the one People & Routing
  // authority saves (Front Desk edits, agent removal, a lane policy save),
  // Memory's routing snapshot, lane statuses, and posture facts must follow
  // the new truth instead of presenting a stale copy.
  useEffect(() => {
    let cancelled = false;
    queueMicrotask(() => {
      if (!cancelled) {
  ...
  return {
    ...
    routingLoading,
    routingError,
    laneStatuses: selectedLaneStatuses,
    laneStatusLoading:
      laneStatusAgentId === selectedAgentId ? laneStatusLoading : false,
    laneStatusError:
      laneStatusAgentId === selectedAgentId ? laneStatusError : null,
    lanePolicySaveKey,
    ...
    bindingKey,
    cardsResponse: selectedCardsResponse,
    cards,
    cardQuery,
    setCardQuery,
    cardStatusFilter,
    setCardStatusFilter,
    episodesResponse: selectedEpisodesResponse,
    episodes,
    episodeQuery,
    setEpisodeQuery,
    graphMapResponse: selectedGraphMapResponse,
    graphNodes,
    graphLinks,
    runtimeHealthResponse: selectedRuntimeHealthResponse,
    telemetrySummaryResponse: selectedTelemetrySummaryResponse,
    telemetrySummary,
    telemetryTurnsResponse: selectedTelemetryTurnsResponse,
    telemetryTurns,
    decisionReasonsResponse: selectedDecisionReasonsResponse,
    decisionReasons,
    selectedCardId,
    setSelectedCardId,
    selectedCard,
