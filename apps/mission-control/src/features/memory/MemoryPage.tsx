...
/**
 * The newest explicit room intent. Entering the stable `memory` room lands on
 * the global plant; a Staff-card entry names the exact agent to open. A seq
 * that has already been applied never re-lands, so internal choices survive
 * unrelated rerenders.
 */
export interface MemoryLandingIntent {
  seq: number;
  kind: "plant" | "agent";
  agentId: string | null;
}
...
export function MemoryPage({ controller, onOpenAssistant, landing }: MemoryPageProps) {
  const [activeSection, setActiveSection] = useState<MemorySection>("plant");
  ...
  // Only a NEW explicit room intent re-lands the page; the same seq applied
  // once never re-applies, so internal choices survive unrelated rerenders.
  // Exact-agent selection happens at the intent source (the event handler),
  // which keeps this a pure local-state derivation.
  const [appliedLandingSeq, setAppliedLandingSeq] = useState(0);
  if (landing && landing.seq !== appliedLandingSeq) {
    setAppliedLandingSeq(landing.seq);
    setActiveSection(
      landing.kind === "agent" && landing.agentId ? "library" : "plant"
    );
  }
  ...
  // Global posture facts for the plant board come straight from the runtime
  // routing snapshot; live health is only ever claimed for the one assistant
  // whose server-backed status has actually been checked in this sitting.
  const enabledHumanIds = new Set(
    (controller.routingConfig?.enabled
      ? controller.routingConfig.human_identities
      : []
    )
      .filter((human) => human.enabled)
      .map((human) => human.human_identity_id)
  );
  const routedCountByAgent = new Map<string, number>();
  for (const assignment of controller.routingConfig?.enabled
    ? controller.routingConfig.assistant_assignments
    : []) {
    if (!assignment.enabled || !enabledHumanIds.has(assignment.human_identity_id)) {
      continue;
    }
    routedCountByAgent.set(
      assignment.assistant_agent_id,
      (routedCountByAgent.get(assignment.assistant_agent_id) ?? 0) + 1
    );
  }
  const policyCountByAgent = new Map<string, number>();
  for (const policy of controller.routingConfig?.lane_memory_policies ?? []) {
    policyCountByAgent.set(
      policy.assistant_agent_id,
      (policyCountByAgent.get(policy.assistant_agent_id) ?? 0) + 1
    );
  }
    ...
    <section className="mc-memory-page" data-testid="memory-page">
      ...
      <div className="mc-page-section-tabs" aria-label="Memory sections">
        {[
          ...
          <button
            ...
            onClick={() => setActiveSection(value as MemorySection)}
          >
            {label}
          </button>
        ))}
        <PinRoomToOffice roomId="memory" />
      </div>
      ...
      {activeSection === "plant" ? (
        <>
          ...
          <Surface
            className="mc-memory-panel mc-memory-lane-map"
            title="Binding Posture"
            subtitle="Every assistant's memory wiring at a glance. Open one to inspect its lanes, cards, and reasoning."
          >
            {controller.agents.length === 0 ? (
              <EmptyState
                className="mc-memory-empty"
                message="No assistants exist yet."
              />
            ) : (
              <div className="mc-memory-lane-card-grid">
                {controller.agents.map((agent) => {
                  const isInspected =
                    agent.agent_id === controller.selectedAgentId &&
                    Boolean(controller.status);
                  const routedCount = routedCountByAgent.get(agent.agent_id) ?? 0;
                  const policyCount = policyCountByAgent.get(agent.agent_id) ?? 0;
                  return (
                    <article key={agent.agent_id} className="mc-memory-lane-card">
                        ...
                        <Chip
                          label={
                            agent.memory_binding?.enabled
                              ? `memory bound · ${agent.memory_binding.provider_kind}`
                              : "no memory binding"
                          }
                          tone={agent.memory_binding?.enabled ? "up" : "checking"}
                        />
                      </div>
                      <div className="mc-memory-chip-row">
                        <Chip
                          label={
                            routedCount > 0
                              ? `${routedCount} routed ${
                                  routedCount === 1 ? "person" : "people"
                                }`
                              : "no one routed yet"
                          }
                          tone={routedCount > 0 ? "up" : "checking"}
                        />
                        <Chip
                          label={
                            policyCount > 0
                              ? `${policyCount} configured lane ${
                                  policyCount === 1 ? "policy" : "policies"
                                }`
                              : "runtime default policy"
                          }
                          tone={policyCount > 0 ? "up" : "checking"}
                        />
                      </div>
                      <div className="mc-memory-chip-row">
                        {isInspected ? (
                          <>
                            <Chip
                              label={`checked: ${
                                controller.status?.binding_status ?? "unknown"
                              }`}
                              tone={toneForBindingStatus(
                                controller.status?.binding_status
                              )}
                            />
                            {managedLaneCount > 0 ? (
                              <Chip
                                label={`${availableLaneCount}/${managedLaneCount} lanes ready`}
                                tone={
                                  availableLaneCount === managedLaneCount
                                    ? "up"
                                    : "warning"
                                }
                              />
                          ...
                          <Chip label="not checked in this sitting" tone="checking" />
                        )}
                      </div>
                      <div className="mc-memory-inline-actions">
                        <button
                          ...
                          className="ghost"
                          onClick={() => {
                            if (agent.agent_id !== controller.selectedAgentId) {
                              controller.setSelectedAgentId(agent.agent_id);
                            }
                            setActiveSection("library");
                          }}
                        >
                          <Brain size={14} /> Inspect memory
                        </button>
                      </div>
                    </article>
                  );
