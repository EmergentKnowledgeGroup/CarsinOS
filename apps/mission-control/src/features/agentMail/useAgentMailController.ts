...
export function useAgentMailController(options: UseAgentMailControllerOptions) {
  ...
  const [leasesIdentity, setLeasesIdentity] = useState<string | null>(null);
  ...
  const leaseReadIdentity = [
    tokenConfigured ? "authenticated" : "unauthenticated",
    settings.gateway_url.trim(),
    mailPrincipalOverride.trim(),
  ].join("\u0000");

  const agentMailRefreshTimer = useRef<number | null>(null);
  const mailThreadLoadSeq = useRef(0);
  const roomThreadLoadSeq = useRef(0);
  // Every lease read carries a request generation: a response older than the
  // newest principal/settings/auth identity may never overwrite newer truth.
  const leaseRequestSeq = useRef(0);
  // Synchronous mutation locks. React render state cannot lock same-tick
  // duplicates, and UI-level busy flags are presentation, not the authority.
  const createLeaseLockRef = useRef(false);
  const releaseLeaseLocksRef = useRef<Set<string>>(new Set());
  const reserveRoomWorkspaceLockRef = useRef(false);
  ...
  const loadFileLeases = useCallback(
    async (runtimeSettings: RuntimeConnectionSettings = settings) => {
      if (!runtimeSettings.gateway_url.trim() || !tokenConfigured) {
        return;
      }
      ...
      const requestSeq = ++leaseRequestSeq.current;
      // Keep same-identity facts stable during an ordinary refresh so local
      // pagination does not reset. Identity transitions are cleared by the
      // identity effect below before their debounced replacement starts.
      setLeasesError(null);
      setLeasesIdentity(requestIdentity);
      setLeasesLoading(true);
      try {
        const items = await listAgentMailFileLeases(runtimeSettings, {
          holderPrincipal: mailPrincipalOverride.trim() || undefined,
          includeReleased: false,
        });
        if (requestSeq !== leaseRequestSeq.current) {
          return;
        }
        setLeases(items);
    ...
    [mailPrincipalOverride, settings, tokenConfigured]
  );
  ...
  const loadAgentMailReadModels = useCallback(
    async (runtimeSettings: RuntimeConnectionSettings = settings) => {
      ...
      // Leases load beside the thread reads but fail independently: a lease
      // outage may not erase or falsify Direct Mail and Agent-room truth, and
      // a thread outage may not erase committed lease truth.
      const leasePart = loadFileLeases(runtimeSettings);
      try {
        const [directThreads, roomThreadItems] = await Promise.all([
    ...
    [loadFileLeases, mailPrincipalOverride, mailSearch, mailboxFilter, settings]
  );
  ...
  useEffect(() => {
    // Principal, gateway, and authentication changes invalidate in-flight
    // reads before the debounced replacement starts. The identity tag also
    // hides the prior list on the transition render.
    leaseRequestSeq.current += 1;
    setLeases([]);
    setLeasesLoaded(false);
    setLeasesLoading(false);
    setLeasesError(null);
    setLeasesIdentity(leaseReadIdentity);
  }, [leaseReadIdentity]);
  ...
  const releaseFileLease = useCallback(
    async (leaseId: string) => {
      // One entity lock per lease: a same-tick duplicate release of one lease
      // is refused while an unrelated lease keeps its own release path.
      if (releaseLeaseLocksRef.current.has(leaseId)) {
        return false;
      }
      releaseLeaseLocksRef.current.add(leaseId);
      try {
        ...
        releaseLeaseLocksRef.current.delete(leaseId);
      }
    },
  ...
  const reserveSelectedRoomWorkspace = useCallback(async () => {
    if (!selectedRoomThreadIdEffective || reserveRoomWorkspaceLockRef.current) {
      return;
    }
    reserveRoomWorkspaceLockRef.current = true;
    try {
      ...
      reserveRoomWorkspaceLockRef.current = false;
    }
  }, [
    leaseHolderPrincipal,
    queueAgentMailRefresh,
  ...
  const leaseFactsAreCurrent = leasesIdentity === leaseReadIdentity;
  ...
  return {
    ...
    leases: leaseFactsAreCurrent ? leases : [],
    leasesLoading: leaseFactsAreCurrent ? leasesLoading : false,
    leasesLoaded: leaseFactsAreCurrent ? leasesLoaded : false,
    leasesError: leaseFactsAreCurrent ? leasesError : null,
    refreshLeases: loadFileLeases,
    leaseHolderPrincipal,
