...
export function EventsPage(props: EventsPageProps) {
  ...
  const clampedEventsPage = pagination.clampPage(eventsPage);
  if (clampedEventsPage !== eventsPage) {
    ...
    // a live feed shrinks and then grows again.
    setEventsPage(clampedEventsPage);
  }
  const pageItems = pagination.getPage(clampedEventsPage);
  ...
  return (
    <section className="mc-alt-grid">
      ...
      >
        {/* The pin rides the shared room-row idiom beside the filter chips;
            Show heartbeats keeps the header slot untouched, and the pin owns
            no feed, filter, page, or disclosure state. */}
        <div className="mc-room-tabs-row mc-events-room-row">
        ...
        <Pagination currentPage={clampedEventsPage} totalPages={pagination.totalPages} onPageChange={setEventsPage} />
      </Surface>
    </section>
  );
}
