...
interface TeamPageProps {
  ...
  /**
   * The resolved stable room id from the elevator. Two rooms share the team
   * route: Trenches "staff" must land on Agents and Basement "models" on the
   * Models & Providers surface. Any other value leaves the section alone.
   */
  activeRoomId: string | null;
  /** Authoritative configured auth profiles, read-only in this surface. */
  authProfiles: AuthProfileResponse[];
  /**
   * The single shared People & Routing authority. Team reads routed-people
   * facts and delegates agent-removal cleanup here; the editable surface
   * lives in Directory / Front Desk.
   */
  peopleRouting: PeopleRoutingController;
  /** Stable-room handoff to Directory / Front Desk (room id "directory"). */
  onOpenPeopleRouting: () => void;
  /**
   * Stable-room handoff into Memory plant that opens this exact agent's
   * memory drill-in. Absent when the memory room is unavailable, so the
   * card never shows a dead control.
   */
  onOpenAgentMemory?: (agentId: string) => void;
}

/**
 * Landing section per stable room id. Only the two rooms sharing the team
 * route are listed; no route-name or display-label guesses.
 */
const ROOM_LANDING_SECTION: Readonly<Partial<Record<string, TeamSection>>> = {
  staff: "agents",
  models: "models",
};

type TeamSection = "agents" | "presets" | "org" | "models";
...
export function TeamPage({
  ...
  const [activeSection, setActiveSection] = useState<TeamSection>(
    () =>
      (activeRoomId ? ROOM_LANDING_SECTION[activeRoomId] : undefined) ??
      "agents",
  );
  ...
  const [lastRoomId, setLastRoomId] = useState(activeRoomId);
  if (activeRoomId !== lastRoomId) {
    setLastRoomId(activeRoomId);
    const landing = activeRoomId
      ? ROOM_LANDING_SECTION[activeRoomId]
      : undefined;
    if (landing) {
  ...
  const [teamNotice, setTeamNotice] = useState<TeamNoticeState | null>(null);
    ...
    async (agent: Agent) => {
      ...
      if (routingDirty) {
        setTeamNotice({
          tone: "error",
          message:
            "People & Routing has unsaved changes. Save or reset them in Directory / Front Desk before removing an agent so those edits are not discarded.",
        });
        return;
      }
      ...
      setTeamNotice(null);
      let routingBeforeDetach: Awaited<
        ReturnType<PeopleRoutingController["detachAgentRouting"]>
      >;
      try {
        routingBeforeDetach = await detachAgentRouting(agent.agent_id);
      } catch (routingError: unknown) {
        setTeamNotice({
          tone: "error",
          message: `Removing ${agent.name} was stopped before the agent changed because People & Routing could not be prepared. ${String(routingError)}`,
        });
      ...
      try {
        ...
        try {
          await restoreRoutingConfig(routingBeforeDetach);
          setTeamNotice({
            tone: "error",
            message: `${friendlyRemoveAgentError(removeError)} People & Routing was restored to its exact prior state.`,
          });
        } catch (restoreError: unknown) {
          setTeamNotice({
            tone: "error",
            message: `${friendlyRemoveAgentError(removeError)} Routing was detached first and its automatic restoration also failed (${String(restoreError)}). Open Directory / Front Desk and restore that agent's assignments before retrying.`,
          });
        }
        setDeletingAgentId(null);
        return;
      ...
      try {
        ...
        setTeamNotice({
          tone: "error",
          message: `${agent.name} was removed, but refreshing Team and People & Routing failed. Refresh both surfaces before making another change. (${String(refreshError)})`,
        });
      } finally {
        setDeletingAgentId(null);
      }
  ...
  return (
    <div className="mc-team-page" data-testid="team-page">
        ...
        <div className="mc-team-header">
          <div>
            ...
            <p className="mc-team-stats">
              <span>
                {authProfiles.length} auth profile
                {authProfiles.length !== 1 ? "s" : ""}
              </span>
      ...
      <div className="mc-room-tabs-row">
        <div className="mc-page-section-tabs" aria-label="Team sections">
          ...
          {/* People & Routing now lives in Basement · Directory / Front Desk.
              This affordance stays for reachability and hands off by stable
              room id instead of mounting a second editable truth here. */}
          <button
            type="button"
            className="mc-page-section-btn"
            onClick={onOpenPeopleRouting}
            title="Opens Directory / Front Desk"
          >
            People & Routing
          </button>
          {strategyEnabled ? (
          ...
          <button
            type="button"
            className={`mc-page-section-btn${activeSection === "models" ? " mc-page-section-btn-active" : ""}`}
            onClick={() => setActiveSection("models")}
          >
            Models &amp; Providers
          </button>
        </div>
        {/* Each pin belongs to its own room identity: two rooms share this
            surface, and one door must never ambiguously open both. */}
        {activeRoomId === "staff" ? <PinRoomToOffice roomId="staff" /> : null}
        {activeRoomId === "models" ? (
          <PinRoomToOffice roomId="models" />
        ) : null}
      </div>
      ...
      {activeSection === "agents" && teamNotice ? (
        <div
          className={clsx(
            "mc-notice",
            teamNotice.tone === "error" ? "mc-notice-error" : "mc-notice-info"
          )}
        >
          {teamNotice.message}
        </div>
                ...
                <div className="mc-team-card-actions">
                  ...
                  {onOpenAgentMemory ? (
                    <button
                      type="button"
                      className="mc-btn mc-btn-sm"
                      onClick={() => onOpenAgentMemory(agent.agent_id)}
                      title={`Open ${agent.name}'s memory in the Memory plant`}
                    >
                      <Brain size={14} />
                      Memory
                    </button>
