...
interface ExecassRequestOptions {
  ...
  ownerProof?: string;
  /**
   * Explicit x-request-id. Owner-proof mutations must send the binding's
   * request_correlation_id here - the gateway rebuilds the expected binding
   * from this header and rejects any mismatch.
   */
  requestId?: string;
...
export async function updateExecassPolicy(
  ...
  return execassRequest(settings, `${API_PREFIX}/policy`, {
    ...
    requestId: authorization.binding.request_correlation_id,
