...
describe("roomForTab", () => {
  ...
  test("registers Policy as a stable Basement room on the shared connectors route", () => {
    const policy = findRoom(DEFAULT_FLOORS, "policy");
    expect(policy?.floor.id).toBe("basement");
    expect(policy?.room.route).toBe("connectors");
    expect(policy?.room.blocks).toEqual(["policy"]);
    // The stable id - not the shared route - owns resolution and relanding.
    expect(roomForTab(DEFAULT_FLOORS, "connectors", "policy")?.room.id).toBe(
      "policy",
    );
    expect(roomForTab(DEFAULT_FLOORS, "connectors")?.room.id).toBe(
      "connectors",
