...
function createAgentMailThreadFixture({
  kind,
  subject,
  createdBy = "agent-root",
  participants = ["agent-root", "default"],
  ageMs,
}) {
  ...
  return {
    ...
    kind,
    subject,
    created_by_principal: createdBy,
    participant_count: new Set([createdBy, ...participants]).size,
    message_count: 0,
...
const agentMailParticipantsByThread = new Map(
  agentMailThreads.map((thread) => [
    thread.thread_id,
    [
      {
...
// Five incoming and four outgoing threads make inbox/outbox/principal filters
// observable instead of returning the same false dataset for every query.
const agentMailMessagesByThread = new Map();
agentMailThreads.forEach((thread, index) => {
  const incomingForRoot = index < 5;
  const createdAt = thread.updated_at;
  const sender = incomingForRoot ? "default" : "agent-root";
  const recipient = incomingForRoot ? "agent-root" : "default";
  const message = {
    message_id: `mail-msg-${nextAgentMailCounter++}`,
    thread_id: thread.thread_id,
    sender_principal: sender,
    sender_kind: "agent",
    body_text: `Latest note for ${thread.subject}`,
    metadata_json: null,
    created_at: createdAt,
    recipients: [
      {
  ...
  agentMailMessagesByThread.set(thread.thread_id, [message]);
  thread.message_count = 1;
  thread.latest_message_at = createdAt;
  thread.latest_message_preview = message.body_text;
  thread.latest_sender_principal = sender;
  thread.unread_count = incomingForRoot ? 1 : 0;
});
...
const seedState = {
  ...
  agentMailParticipants: Object.fromEntries(
    Array.from(
      agentMailParticipantsByThread.entries(),
      ([threadId, participants]) => [threadId, cloneSeed(participants)],
    ),
  ),
...
function resetExecassState() {
  ...
  execassState = {
    ...
    resolved: {},
    extraDelegations: [],
    // Stateful owner policy: the mock enforces the same binding identity as
    // the production gateway (recomputed body and safe-snapshot digests plus
    // the header-bound correlation id) and records every PUT exactly.
    policy: {
      revision: 7,
      configured: true,
      profile: "balanced",
      ...
      updatedAtMs: Date.now() - 24 * 3_600_000,
    },
    policyRequests: [],
    policyFailureMode: "none",
  };
}
...
// ── Policy binding identity, mirrored from the production gateway ──

function sha256HexUtf8(text) {
  return createHash("sha256").update(text, "utf8").digest("hex");
}

function policyOrderedQuota(quota) {
  return {
    kind: quota.kind,
    limit: quota.limit,
...
function policyOrderedRule(rule) {
  return {
    ...
    technical_resource_quotas: (rule.technical_resource_quotas ?? []).map(
      policyOrderedQuota,
    ),
...
/** serde struct-order serialization of the parsed PolicyUpdateRequest. */
function policyBodyJson(body) {
  return JSON.stringify({
    idempotency_key: body.idempotency_key,
    expected_policy_revision: body.expected_policy_revision,
    proposed_profile: body.proposed_profile,
    proposed_rules: (body.proposed_rules ?? []).map(policyOrderedRule),
    change_summary: body.change_summary,
  });
}

function writePolicyCanonical(value, out) {
  if (value === null) {
    out.push("null");
    return;
  }
  ...
  if (typeof value === "number") {
    if (!Number.isSafeInteger(value)) {
      throw new Error("canonical policy JSON requires safe integers");
    }
    out.push(String(value));
    return;
  }
  if (typeof value === "string") {
    out.push(JSON.stringify(value.normalize("NFC")));
    return;
  }
  if (Array.isArray(value)) {
    ...
    value.forEach((entry, index) => {
      if (index !== 0) out.push(",");
      writePolicyCanonical(entry, out);
    });
    out.push("]");
    return;
  }
  ...
  keys.forEach((key, index) => {
    ...
    out.push(":");
    writePolicyCanonical(value[key], out);
  });
  out.push("}");
}

/** Strict canonical safe snapshot exactly as put_policy constructs it. */
function policyCanonicalSnapshotJson(body) {
  const out = [];
  writePolicyCanonical(
    {
      configured: true,
      profile: body.proposed_profile,
      rules: (body.proposed_rules ?? []).map(policyOrderedRule),
      effective_operational_summary: body.change_summary,
    },
    out,
  );
  return out.join("");
...
/** One stable field order so binding equality is order-independent. */
function policyOrderedBinding(binding) {
  return {
    ...
    canonical_body_digest: binding.canonical_body_digest,
    safe_snapshot_digest: binding.safe_snapshot_digest,
    request_correlation_id: binding.request_correlation_id,
    created_at_ms: binding.created_at_ms,
  };
}

function decodeOwnerProofHeader(headerValue) {
  try {
...
function policyResponseJson() {
  const policy = execassState.policy;
  return {
    policy_id: "execass-policy",
    configured: policy.configured,
    effective_operational_summary: policy.effectiveSummary,
    revision: policy.revision,
    rules: policy.rules.map(policyOrderedRule),
    updated_at_ms: policy.updatedAtMs,
...
function handleExecassResume(socket, request) {
  ...
  // Events before the boot base are compacted away, exactly like a real
  // gateway with a bounded event log: a fresh or ancient cursor cannot be
  // silently accepted (live events would gap forever) - it must refetch
  // and resume from the disclosed head.
  const retentionBase = 1000;
  if (
    !Number.isFinite(cursor) ||
    cursor < 0 ||
    cursor > head ||
    cursor < retentionBase
  ) {
    socket.send(
      JSON.stringify({
...
function resetMockState() {
  ...
  agentMailParticipantsByThread.clear();
  for (const [threadId, participants] of Object.entries(
    seedState.agentMailParticipants,
  )) {
    agentMailParticipantsByThread.set(threadId, cloneSeed(participants));
  }
  nextAgentMailCounter = 1000;
  replaceArray(agentMailLeases, createAgentMailLeaseFixtures());
  providerOrder.clear();
...
async function routeRequest(req, res) {
  ...
  if (req.method === "GET" && requestUrl.pathname === "/api/v1/execass/policy") {
    sendJson(res, 200, policyResponseJson());
    return;
  }

  if (req.method === "PUT" && requestUrl.pathname === "/api/v1/execass/policy") {
    const body = await readJson(req);
    const proofHeaderRaw = req.headers["x-execass-owner-proof"];
    const requestId = String(req.headers["x-request-id"] ?? "execass-request");
    execassState.policyRequests.push({
      method: "PUT",
      path: requestUrl.pathname,
      idempotency_header: String(req.headers["idempotency-key"] ?? ""),
      request_id: requestId,
      owner_proof_header: proofHeaderRaw ? String(proofHeaderRaw) : null,
      body,
      received_at_ms: Date.now(),
    });
    if (execassState.policyFailureMode === "unavailable") {
      execassState.policyFailureMode = "none";
      execassApiError(
        res,
        503,
        "execass.v1.external_dependency",
        "The policy service is briefly unavailable.",
        true,
      );
      return;
    }
    ...
    const authorization = proofHeaderRaw
      ? decodeOwnerProofHeader(proofHeaderRaw)
      : null;
    if (!authorization?.binding || !authorization?.proof) {
      execassApiError(
        res,
        403,
    ...
    const expectedBinding = {
      ...
      expected_revision: Number(body.expected_policy_revision ?? 0),
      canonical_body_digest: sha256HexUtf8(policyBodyJson(body)),
      safe_snapshot_digest: sha256HexUtf8(policyCanonicalSnapshotJson(body)),
      request_correlation_id: requestId,
      created_at_ms: authorization.binding.created_at_ms,
    };
    const suppliedBinding = { ...authorization.binding };
    const bindingMatches =
      JSON.stringify(policyOrderedBinding(suppliedBinding)) ===
        JSON.stringify(policyOrderedBinding(expectedBinding)) &&
      authorization.proof.request_correlation_id ===
        expectedBinding.request_correlation_id;
    if (!bindingMatches) {
      execassApiError(
        res,
        403,
    ...
    if (execassState.policyFailureMode === "conflict") {
      execassState.policyFailureMode = "none";
      // The simulated concurrent writer that caused this conflict.
      execassState.policy.revision += 1;
      execassState.policy.effectiveSummary =
        "Externally revised while you were editing.";
      execassState.policy.rules = execassState.policy.rules.map((rule) =>
        rule.rule_id === "rule-1"
          ? { ...rule, clarification_sensitivity: "strict" }
          : rule,
      );
      broadcastExecassEvent("execass.v1.policy.changed", {
    ...
    if (
      Number(body.expected_policy_revision) !== execassState.policy.revision
    ) {
      execassApiError(
        res,
        409,
    ...
    execassState.policy.revision += 1;
    execassState.policy.configured = true;
    execassState.policy.profile = body.proposed_profile;
    execassState.policy.rules = (body.proposed_rules ?? []).map(
      policyOrderedRule,
    );
    execassState.policy.effectiveSummary = String(body.change_summary ?? "");
    execassState.policy.updatedAtMs = Date.now();
    broadcastExecassEvent("execass.v1.policy.changed", {
      summary: "Policy changed",
    });
    sendJson(res, 200, {
      policy: policyResponseJson(),
      updated_at_ms: execassState.policy.updatedAtMs,
    });
    return;
  }
  ...
  if (
    req.method === "POST" &&
    requestUrl.pathname === "/api/v1/memory/sync"
  ) {
    const payload = await readJson(req);
    const humanId =
  ...
  if (req.method === "GET" && requestUrl.pathname === "/api/v1/agent-mail/threads") {
    ...
    const mailbox = (requestUrl.searchParams.get("mailbox") ?? "all").toLowerCase();
    const principal =
      (requestUrl.searchParams.get("principal_id") ?? "agent-root").trim() ||
      "agent-root";
    ...
    const items = agentMailThreads
      ...
      .filter((thread) =>
        (agentMailParticipantsByThread.get(thread.thread_id) ?? []).some(
          (participant) => participant.principal_id === principal,
        ),
      )
      .filter((thread) => {
        const messages = agentMailMessagesByThread.get(thread.thread_id) ?? [];
        if (mailbox === "inbox") {
          return messages.some((message) =>
            message.recipients.some(
              (recipient) => recipient.recipient_principal === principal,
            ),
          );
        }
        if (mailbox === "outbox") {
          return messages.some(
            (message) => message.sender_principal === principal,
          );
        }
        return true;
      })
      .filter((thread) => {
        ...
        return (
          ...
          messages.some((message) => message.body_text.toLowerCase().includes(search))
        );
      })
      .slice(0, limit)
      .map((thread) => {
        ...
        const unreadCount = messages.reduce(
          (count, message) =>
            ...
            message.recipients.filter(
              (recipient) =>
                recipient.recipient_principal === principal &&
                recipient.acked_at === null,
            ).length,
          0,
        );
  ...
  if (req.method === "POST" && requestUrl.pathname === "/api/v1/agent-mail/threads") {
    ...
    if (!["direct", "room"].includes(kind)) {
      sendJson(res, 400, { error: "thread kind must be direct or room" });
      return;
    }
    if (!subject || subject.length > 240) {
      sendJson(res, 400, { error: "thread subject must be 1-240 characters" });
      return;
    }
    ...
    agentMailParticipantsByThread.set(
      thread.thread_id,
      Array.from(new Set(["agent-root", ...participants])).map(
        (principalId, index) => ({
...

  {
    ...
    if (threadDetailMatch && req.method === "GET") {
      ...
      sendJson(res, 200, {
        thread,
        participants:
          agentMailParticipantsByThread.get(thread.thread_id) ?? [],
      });
      return;
    }
...

  {
    ...
    if (threadMessagesMatch) {
      ...
      if (req.method === "POST") {
        ...
        if (!bodyText) {
          sendJson(res, 400, { error: "message body cannot be empty" });
          return;
        }
        ...
        const message = {
          ...
          metadata_json: null,
          created_at: Date.now(),
          recipients: recipients.map((principal) => ({
  ...
  if (req.method === "GET" && requestUrl.pathname === "/api/v1/agent-mail/leases") {
    ...
    // Production trims the holder filter and ignores a blank value; the mock
    // must enforce the same identity boundary rather than returning one
    // convenient dataset for every acting principal.
    const holderParam = requestUrl.searchParams.get("holder_principal");
    const holder =
      typeof holderParam === "string" && holderParam.trim()
        ? holderParam.trim()
        : null;
    sendJson(res, 200, {
      items: agentMailLeases.filter(
        (lease) =>
          (includeReleased ? true : lease.released_at === null) &&
          (holder === null || lease.holder_principal === holder),
      ),
    });
    return;
  }
  ...
  if (req.method === "POST" && requestUrl.pathname === "/api/v1/agent-mail/leases") {
    const payload = await readJson(req);
    const ttlMs = payload.ttl_ms;
    const globPattern =
      typeof payload.glob_pattern === "string" ? payload.glob_pattern.trim() : "";
    if (!Number.isInteger(ttlMs) || ttlMs <= 0) {
      sendJson(res, 400, { error: "ttl_ms must be a positive integer" });
      return;
    }
    if (!globPattern) {
      sendJson(res, 400, { error: "glob_pattern must not be empty" });
      return;
    }
    const lease = {
...

  {
    ...
    if (req.method === "POST" && releaseMatch) {
      ...
      const requestedHolder =
        typeof payload.holder_principal === "string" &&
        payload.holder_principal.trim()
          ? payload.holder_principal.trim()
          : null;
      if (
        ...
        (requestedHolder !== null &&
          requestedHolder !== lease.holder_principal)
      ) {
  ...
  if (
    req.method === "GET" &&
    requestUrl.pathname === "/api/v1/e2e/execass-policy-requests"
  ) {
    sendJson(res, 200, { items: execassState.policyRequests });
    return;
  }

  if (
    req.method === "POST" &&
    requestUrl.pathname === "/api/v1/e2e/execass-policy-changed"
  ) {
    const payload = await readJson(req);
    execassState.policy.revision += 1;
    if (typeof payload.summary === "string" && payload.summary.trim()) {
      execassState.policy.effectiveSummary = payload.summary.trim();
    }
    if (typeof payload.profile === "string" && payload.profile.trim()) {
      execassState.policy.profile = payload.profile.trim();
    }
    execassState.policy.updatedAtMs = Date.now();
    broadcastExecassEvent("execass.v1.policy.changed", {
      summary: "Policy changed",
    });
    sendJson(res, 200, { revision: execassState.policy.revision });
    return;
  }

  if (
    req.method === "POST" &&
    requestUrl.pathname === "/api/v1/e2e/execass-policy-failure"
  ) {
    const payload = await readJson(req);
    const mode = String(payload.mode ?? "none");
    if (!["none", "unavailable", "conflict"].includes(mode)) {
      sendJson(res, 400, { error: "unknown policy failure mode" });
      return;
    }
    execassState.policyFailureMode = mode;
