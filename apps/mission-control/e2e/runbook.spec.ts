...
test.describe("mission-control runbook @core", () => {
  test("hides Runbook until enabled and renders the shared runbook surface", async ({
    ...
    const requestedApproval = page.locator(".mc-focus-item", {
      hasText: "Approval requested: Allow shell command: ls -la",
    });
    await expect(requestedApproval).toBeVisible();
    await expect(requestedApproval).toContainText(
      "Approval approval-001 requires operator decision.",
    );
    await expect(requestedApproval).toContainText("Hide details");
    await expect(requestedApproval).toContainText("Allow shell command: ls -la");
  });
