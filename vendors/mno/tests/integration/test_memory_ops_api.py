...
def test_wizard_path_sanitizer_collapses_mangled_windows_mount_paths(monkeypatch) -> None:
    ...
    normalized = runtime_server_module._wizard_normalize_path_value(
        r"Z:\mnt\z\modelNumquamOblita\runtime\imports\atoms.sqlite3"
    )
    assert normalized == r"Z:\modelNumquamOblita\runtime\imports\atoms.sqlite3"

    state = {
        "store_path": r"Z:\mnt\z\modelNumquamOblita\runtime\imports\atoms.sqlite3",
        "store_validation": {
            "path": r"Z:\mnt\z\modelNumquamOblita\runtime\imports\atoms.sqlite3",
        },
        "published_set": {
            "episodes_path": r"Z:\modelNumquamOblita\runtime\episodes\episode_cards.reviewed.json",
        },
        "verify": {
            "checks": [
                {"id": "store_validation", "path": r"Z:\mnt\z\modelNumquamOblita\runtime\imports\atoms.sqlite3"},
                {"id": "published_set", "path": r"Z:\modelNumquamOblita\runtime\episodes\episode_cards.reviewed.json"},
            ],
            "actionable_links": [],
        },
    }
    sanitized = runtime_server_module._wizard_normalize_state_paths(state)
    assert sanitized["store_path"] == r"Z:\modelNumquamOblita\runtime\imports\atoms.sqlite3"
    assert sanitized["store_validation"]["path"] == r"Z:\modelNumquamOblita\runtime\imports\atoms.sqlite3"
    assert sanitized["verify"]["checks"][0]["path"] == r"Z:\modelNumquamOblita\runtime\imports\atoms.sqlite3"
