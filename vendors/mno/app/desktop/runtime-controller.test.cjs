...
test('normalizePlatformPath converts WSL mount paths into native Windows paths', () => {
  assert.equal(
    normalizePlatformPath('/mnt/z/modelNumquamOblita/runtime/imports/atoms.sqlite3', { platform: 'win32' }),
    'Z:\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3',
  );
  assert.equal(
    normalizePlatformPath('Z:\\mnt\\z\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3', { platform: 'win32' }),
    'Z:\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3',
  );
  assert.equal(
    normalizePlatformPath('Z:\\modelNumquamOblita\\runtime\\episodes\\episode_cards.reviewed.json', { platform: 'win32' }),
    'Z:\\modelNumquamOblita\\runtime\\episodes\\episode_cards.reviewed.json',
  );
});
...
test('buildRuntimeLaunchPlan normalizes WSL-authored wizard paths on Windows', () => {
  ...
  const plan = buildRuntimeLaunchPlan({
    repoRoot: 'Z:\\modelNumquamOblita',
    runtimeManifest: manifest,
    platform: 'win32',
    memories: '/mnt/z/modelNumquamOblita/runtime/imports/atoms.sqlite3',
    episodes: '/mnt/z/modelNumquamOblita/runtime/episodes/episode_cards.reviewed.json',
  });
  assert.ok(plan.args.includes('Z:\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3'));
  assert.ok(plan.args.includes('Z:\\modelNumquamOblita\\runtime\\episodes\\episode_cards.reviewed.json'));
  assert.equal(plan.args.includes('Z:\\mnt\\z\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3'), false);
});
...
test('buildWindowsDetachedRuntimeLauncher wraps the runtime command in Start-Process', () => {
  const launcher = buildWindowsDetachedRuntimeLauncher({
    command: 'python',
    args: ['Z:\\modelNumquamOblita\\tools\\run_live_runtime.py', '--port', '7340'],
    cwd: 'Z:\\modelNumquamOblita',
    stdoutPath: 'Z:\\modelNumquamOblita\\runtime\\desktop_shell\\runtime.stdout.log',
    stderrPath: 'Z:\\modelNumquamOblita\\runtime\\desktop_shell\\runtime.stderr.log',
  });
...
test('buildExpectedBinding normalizes WSL-authored wizard paths for Windows shells', () => {
  const binding = buildExpectedBinding(makeWizardState({
    store_validation: {
      kind: 'mno_store_sqlite',
      is_valid: true,
      path: '/mnt/z/modelNumquamOblita/runtime/imports/atoms.sqlite3',
      store_fingerprint: 'store_fingerprint_v1',
    },
    published_set: {
      episodes_path: '/mnt/z/modelNumquamOblita/runtime/episodes/episode_cards.reviewed.json',
      build_id: 'build_123',
    },
  }), { platform: 'win32' });
  assert.equal(binding.store_path, 'Z:\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3');
  assert.equal(binding.episodes_path, 'Z:\\modelNumquamOblita\\runtime\\episodes\\episode_cards.reviewed.json');
});
...
test('buildExpectedBinding collapses already-mangled Windows mount paths for Windows shells', () => {
  const binding = buildExpectedBinding(makeWizardState({
    store_validation: {
      kind: 'mno_store_sqlite',
      is_valid: true,
      path: 'Z:\\mnt\\z\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3',
      store_fingerprint: 'store_fingerprint_v1',
    },
    published_set: {
      episodes_path: 'Z:\\modelNumquamOblita\\runtime\\episodes\\episode_cards.reviewed.json',
      build_id: 'build_123',
    },
  }), { platform: 'win32' });
  assert.equal(binding.store_path, 'Z:\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3');
...
test('summarizeRuntimeLock matches Windows runtime lock paths against WSL-authored wizard bindings', () => {
  const matching = summarizeRuntimeLock({
    ...
    host: '127.0.0.1',
    port: 7340,
    store_path: 'Z:\\modelNumquamOblita\\runtime\\imports\\atoms.sqlite3',
    store_fingerprint: 'store_fingerprint_v1',
    episodes_path: 'Z:\\modelNumquamOblita\\runtime\\episodes\\episode_cards.reviewed.json',
  }, {
    expectedBinding: buildExpectedBinding(makeWizardState({
