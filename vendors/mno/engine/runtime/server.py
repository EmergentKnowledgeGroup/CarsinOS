...
def _wizard_draft_context_bundle(
    ...
    *,
    ...
    if str((state.get("selected_input") or {}).get("kind") or "").strip() != "ia_archive" or not archive_path:
        return {
            "partial": True,
...
def _wizard_bind_input_to_state(
    ...
    *,
    ...
    if bind_store_validation and str(selected.get("kind") or "") in WIZARD_VALID_STORE_KINDS and bool(selected.get("is_valid")):
        store_validation = _wizard_empty_store_validation()
...
def _wizard_require(state: Mapping[str, Any], *, action: str) -> None:
    ...
    if action == "draft_curation_mcp":
        if not bool(store_validation.get("is_valid")):
            raise ValueError("Import or select a valid MNO memory store before connecting Claude to this draft.")
        if not str(build_info.get("draft_path") or "").strip():
            raise ValueError("Build draft episode cards before connecting Claude to this draft.")
        return
...
def _wizard_draft_curation_mcp_gate_issues(state: Mapping[str, Any]) -> list[str]:
    ...
    store_validation = dict(state.get("store_validation") or {})
    if not bool(store_validation.get("is_valid")):
        issues.append("Import or select a valid MNO memory store before connecting Claude to this draft.")
    source_path, _payload, cards = _wizard_draft_cards_payload(dict(state))
    if source_path is None or not cards:
        issues.append("Build draft episode cards before connecting Claude to this draft.")
    return issues
    ...
    def _integration_handle_request(self, *, method: str, parsed: Any) -> bool:
        ...
        try:
            ...
            if operation == "context.build":
                ...
                window_snapshot = ""
                history_rows = []
                context_session_id = session_id or None
                if context_session_id is not None:
                    try:
                        history_rows = self.server.runtime.get_session_history(context_session_id)
                    except KeyError:
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path in {
            ...
            try:
                ...
                run_id = str(data.get("run_id") or "").strip() or None
                target = str(data.get("target") or "claude_code").strip().lower() or "claude_code"
                if target not in {"claude_code", "claude_desktop"} and path != "/api/wizard/draft-curation/mcp/export":
                    return _json_response(self, HTTPStatus.BAD_REQUEST, {"error": "target must be claude_code or claude_desktop"})
                state = _load_or_create_wizard_state(run_id=run_id, start_new=False)
                ...
                if path == "/api/wizard/draft-curation/mcp/export":
                    ...
                    return _json_response(
                        self,
                        HTTPStatus.OK,
                        {"ok": True, "run_id": state.get("run_id"), "export": export_result, "mcp": draft_state.get("mcp")},
                    )
