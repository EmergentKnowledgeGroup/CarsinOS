...
function currentWizardUiInputPath() {
  ...
  return String(state.wizardState?.selected_input?.path || els.wizardArchivePath?.value || "").trim();
}
...
function wizardPrimaryImportLabel() {
  return state.wizardInputMode === "store" ? "Use This Store" : "Create MNO Store";
}
  ...
  if (els.wizardGoLiveConfig) {
    if (runtimeHealthy && verifiedSafe) {
      ...
      els.wizardGoLiveConfig.innerHTML =
        ...
        `<div class="wizard-result-detail">1. Ask one memory question in Chat. 2. Open Memory Check and confirm the right store and reviewed set are attached. 3. If Claude / MCP is installed, run one external recall test before trusting it.</div>` +
    ...
    if (selected.startsWith("mno_store")) {
      ...
      wizardResult(
        els.wizardStoreSummary,
        {
          ...
          detail: "This lane skips archive import and continues from memory data that is already in MNO format.",
      ...
      wizardResult(els.wizardStoreSummary, {
        ...
        detail: "It skips archive import and lets you continue from an existing store.",
...
function renderWizardState() {
    ...
    if (String(selectedInput.kind || "") === "ia_archive") {
      wizardResult(
        els.wizardArchiveSummary,
        {
          ...
          title: "IA archive selected.",
          detail: "MNO can see the archive you picked. Validation runs automatically, but Import is not complete until the store is created.",
          next: "If File Checked is green, click Create MNO Store.",
          meta: `path=${selectedInput.path || "-"} | conversations=${selectedInput.conversation_count || 0} | messages=${selectedInput.message_count || 0} | ${wizardIssueSummary(selectedInput.issues || [])}`,
        }
      );
    } else if (state.wizardInputMode === "archive") {
      wizardResult(els.wizardArchiveSummary, {
        title: "Pick an IA db.json file to start a new memory import.",
        detail: "The app stages the archive locally, validates it, then turns it into a ready-to-build MNO store.",
        next: "Choose the IA export file and wait for File Checked to turn green.",
        tone: "info",
      });
    }
  }
  ...
  if (els.btnWizardImport) {
    els.btnWizardImport.textContent = state.wizardImportPending
      ? (state.wizardInputMode === "store" ? "Binding Store..." : "Creating Store...")
      : wizardPrimaryImportLabel();
    setWizardActionState(els.btnWizardImport, {
      primary: true,
      complete: importComplete,
      disabled: !importValidation.ok || state.wizardImportPending,
    });
  }
    ...
    if (wizardStageComplete("import", payload) && String(storeValidation.path || payload.store_path || "").trim()) {
      wizardResult(els.wizardImportResult, {
        ...
        detail: state.wizardInputMode === "store"
          ...
          : "The IA archive has been turned into a ready-to-build MNO memory store.",
      ...
      wizardResult(els.wizardImportResult, {
        ...
        detail: "Choose either a new IA archive or an existing MNO store.",
...
function renderWizardDraftCurationStatus() {
  ...
  const resultPayload = !payload.draft_ready
    ? {
        tone: "info",
        title: "Claude draft curation unlocks after Build.",
        detail: "Once a draft exists, Claude can propose bounded cleanup and ranking hints here without touching publish or activation.",
    ...
    : {
        ...
        detail: activeLease
          ? "Claude is actively shaping this draft. Suggestions stay separate until you explicitly promote them."
          : proposalCount > 0 || acceptedCount > 0 || rejectedCount > 0
            ? "Claude suggestions exist for this draft. You can accept, reject, or promote them one at a time."
            : "No Claude suggestions exist yet. The default human-only review path is still ready right now.",
        next: activeLease
          ? "Use Refresh Suggestions to watch the queue, or Release Lease if the curator is stuck."
          : "If you want model-aware draft shaping, let Claude call the draft curation tools, then review the queue here.",
...
function renderWizardDraftCurationList() {
  ...
  if (!rows.length) {
    const label = statusFilter === "pending" ? "No pending Claude suggestions are waiting." : "No Claude suggestions match this filter yet.";
...
function renderWizardDraftCurationMcp() {
  ...
  wizardResult(els.wizardDraftCurationMcpStatus, {
    ...
    title: draftReady ? `Connect Claude to this draft: ${friendlyMcpStatus(status)}` : "Connect Claude to this draft: not ready yet",
    detail: issues.length
      ? issues.join(" ")
      : `This installs the same MNO MCP family for draft curation with ${friendlyMcpRole(selectedRole).toLowerCase()} role, strict mode, and mutation tools ${selectedMutations ? "on" : "off"}.`,
    next: draftReady
      ? "Install Claude Code or Claude Desktop here, then tell Claude to work on this run_id."
      : "Finish Import and Build first. Then this panel can wire Claude into the draft-curation lane.",
    meta: `role=${selectedRole} | compat=strict | mutations=${selectedMutations ? "on" : "off"}${selectedScope ? ` | scope=${selectedScope}` : ""}`,
  });
    ...
    renderWizardDraftCurationSignal(
      els.wizardDraftCurationWorkspaceMcpSummary,
      {
        ...
        title: !draftReady
          ...
          : status === "installed"
            ? `Claude connector ready in ${friendlyMcpScope(selectedScope)} scope.`
            : status === "export_ready"
              ? "Draft MCP bundle is ready to export."
              : `Claude connector ${friendlyMcpStatus(status)}.`,
        detail: issues.length
          ? issues.join(" ")
          : draftReady
      ...
      { fallbackLabel: "Claude Connector" },
    );
  }
  ...
  renderWizardMcpTargets(els.wizardDraftCurationMcpTargets, {
    ...
    disabled: !draftReady,
    emptyMessage: draftReady ? "No draft-curation MCP targets detected." : "Build a draft first, then connect Claude here.",
  });
}
...
async function runWizardDraftCurationMcpInstall(target, ownershipAction = "") {
  ...
  wizardResult(els.wizardDraftCurationMcpStatus, {
    tone: "ok",
    title: "Draft-curation MCP updated.",
    detail: `${target} now points at the current draft curation lane${target === "claude_code" ? ` in ${friendlyMcpScope(claudeCodeScope)} scope` : ""}.`,
    next: "Restart Claude if it was already open, then tell it to use this run_id for draft curation.",
...
async function runWizardDraftCurationMcpRemove(target) {
  ...
  wizardResult(els.wizardDraftCurationMcpStatus, {
    tone: "info",
    title: "Draft-curation MCP removed.",
    detail: `${target} no longer points at this draft curation lane${target === "claude_code" ? ` in ${friendlyMcpScope(claudeCodeScope)} scope` : ""}.`,
    next: "Install it again here if you want Claude back on this draft.",
...
async function exportWizardDraftCurationMcp() {
  const payload = await jsonFetch("/api/wizard/draft-curation/mcp/export", {
    method: "POST",
    body: JSON.stringify(wizardDraftCurationMcpRequestBody(state.wizardRunId || undefined)),
  });
  wizardResult(els.wizardDraftCurationMcpStatus, {
    tone: "ok",
    title: "Draft-curation MCP bundle is ready.",
    detail: "Use this when you want Claude or another client to connect to the current draft-cards lane instead of the published runtime.",
    next: "Install the bundle into the external client, then tell it to work on this run_id.",
    meta: `server=${payload.export?.server_name || "-"}`,
  });
  state.wizardDraftCurationMcp = {
    ...(state.wizardDraftCurationMcp || {}),
    mcp: payload.mcp || {},
...
async function applyWizardDraftCurationManagedMcpProfile() {
  ...
  wizardResult(els.wizardDraftCurationMcpStatus, {
    ...
    next: "Restart Claude if it was already open, then refresh the queue or install targets again if needed.",
...
function renderWizardDraftCurationContext(detail = {}) {
  ...
  els.wizardDraftCurationContext.innerHTML =
    ...
    `<div class="wizard-result-detail">${escapeHtml(context.partial ? `Context was capped or unavailable: ${(context.partial_reasons || []).join(", ") || "unknown reason"}.` : "Claude saw only bounded local context for this card, not an open-ended transcript crawl.")}</div>` +
...
async function openWizardDraftCurationProposal(episodeId, returnFocusNode = null) {
  ...
  if (false && els.wizardDraftCurationProposalMeta) {
    wizardResult(els.wizardDraftCurationProposalMeta, {
      ...
      detail: String(proposal.model_identity || "").trim()
        ? `Claude suggestion from ${proposal.model_identity}.`
        : "Claude suggestion for this draft card.",
      next: "Use This In Review copies Claude into the real review path. Dismiss keeps the original draft card.",
  ...
  wizardResult(els.wizardDraftCurationProposalResult, {
    tone: "info",
    title: "Claude suggestions stay separate until you use one in Review.",
    detail: "Use This In Review copies Claude's suggestion into the real review decision for this card.",
    next: "Dismiss Claude Suggestion ignores Claude and keeps the original draft card untouched.",
  });
...
async function updateWizardDraftProposal(action, episodeId) {
  ...
  renderWizardDraftCurationFeedback({
    ...
    detail: action === "promote"
      ? "Claude's suggestion is now copied into the real human review path for this card."
...
async function runWizardDraftProposalBulkAction(action) {
  ...
  if (!targetIds.length) {
    renderWizardDraftCurationFeedback({
      tone: "info",
      title: "No Claude suggestions are ready to use.",
  ...
  renderWizardDraftCurationFeedback({
    tone: "info",
    title: `Using ${targetIds.length} Claude suggestion${targetIds.length === 1 ? "" : "s"}.`,
  ...
  renderWizardDraftCurationFeedback({
    ...
    title: `Used ${completed} Claude suggestion${completed === 1 ? "" : "s"}.`,
    detail: failures.length
      ? `Some items did not update cleanly.`
      : "The Claude suggestions are now in the real review path.",
...
async function startWizard(mode) {
  ...
  if (mode === "new" && els.wizardArchiveFile) {
...
async function uploadWizardArchive(file) {
  ...
  wizardResult(
    els.wizardImportResult,
    {
      ...
      title: "Archive staged locally.",
      detail: "MNO can see the file you picked, but the step is not complete yet.",
      next: "Wait for File Checked to turn green, then click Create MNO Store.",
      meta: `path=${payload.classification?.path || file.name} | ${wizardIssueSummary(payload.classification?.issues || [])}`,
    }
  );
...
async function runWizardImport() {
  ...
  wizardResult(els.wizardImportResult, {
    tone: "info",
    title: state.wizardInputMode === "store" ? "Binding the selected store." : "Creating the MNO store.",
    detail: state.wizardInputMode === "store"
      ? "MNO is binding the existing store into this wizard run."
      : "MNO is importing the archive into a draft memory store for this wizard run.",
  ...
  try {
    ...
    wizardResult(
      ...
      {
        tone: "ok",
        title: state.wizardInputMode === "store" ? "Existing MNO store is ready to use." : "MNO store created successfully.",
        detail: state.wizardInputMode === "store"
          ? "Import is complete because you selected a valid existing store."
          : "The archive has been converted into a real MNO store and the next step is ready.",
        next: "Move to Build Episodes.",
...
async function runWizardBuild() {
  ...
  wizardResult(
    els.wizardBuildResult,
    {
      ...
      next: "Move to Review and decide what survives, or optionally let Claude help curate the draft first.",
...
async function releaseWizardDraftCurationLease(forceRelease = true) {
  ...
  renderWizardDraftCurationFeedback({
    ...
    detail: "Claude can reconnect cleanly if you want to restart the optional lane later.",
...
function renderWizardMcpTargets(container, { mcp = {}, selectedScope = "user", resultNode = null, onInstall, onRemove, emptyMessage = "No MCP targets detected.", disabled = false }) {
  if (!container) {
    return;
  }
  ...
  const rows = Object.entries(targets)
    .map(([targetKey, target]) => {
      ...
      const summary = trimDisplay(issues || "This target is ready for Claude / MCP setup.", compact ? 120 : 220);
...
async function runWizardGoLive() {
  ...
  wizardResult(els.wizardGoLiveResult, {
    ...
    next: "Use Operate to run the live smoke test, or set up Claude / MCP.",
...
async function runWizardMcpInstall(target, ownershipAction = "") {
  ...
  wizardResult(els.wizardGoLiveResult, {
    tone: "ok",
    title: "Claude / MCP setup updated.",
    detail: `${target} now has the latest MNO connection entry${target === "claude_code" ? ` in ${friendlyMcpScope(claudeCodeScope)} scope` : ""}.`,
...
async function runWizardMcpRemove(target) {
  ...
  wizardResult(els.wizardGoLiveResult, {
    tone: "info",
    title: "Claude / MCP entry removed.",
    detail: `${target} no longer has the MNO connection entry installed${target === "claude_code" ? ` in ${friendlyMcpScope(claudeCodeScope)} scope` : ""}.`,
    next: "Use Set Up Claude / MCP again if you want to reconnect it later.",
    meta: `target=${target}${target === "claude_code" ? ` | scope=${claudeCodeScope}` : ""}`,
  });
  renderWizardActivation(payload);
  await refreshWizardState(state.wizardRunId || undefined);
...
async function exportWizardMcp() {
  ...
  wizardResult(els.wizardGoLiveResult, {
    tone: "ok",
    title: "Claude / MCP bundle is ready.",
    detail: "Use this when you want to connect an external client to the live MNO runtime.",
    next: "Install it into Claude, then run one memory recall smoke test.",
    meta: `server=${payload.export?.server_name || "-"}`,
  });
  await refreshWizardActivationStatus(state.wizardRunId || undefined);
}
