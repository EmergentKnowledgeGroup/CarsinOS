...
Navigation is an **elevator** (left rail, glowing floor lamps). The shipped default has four floors. Keyboard: `4/3/2/B` (or configurable positional shortcuts), `Cmd/Ctrl+K` command palette ("the intercom") from anywhere. Floors are not hardcoded shell branches: a floor registry controls stable ID, label, order, icon/lamp, shortcut, default room, capability requirements, and visible rooms. A floor or room can be added, removed, hidden, reordered, or renamed through registry/config changes without rewriting the application shell.
...
### 3F — The Window
The aquarium. **Reef** (left): ambient canvas of the crew — crabs scuttling about their work; touch a crab, it reports (coarse state + deep link). Truthful, privacy-preserving, zero obligations — the screen you leave on while playing Palworld. **Office Chatter** (right): a channel viewport showing agents' short action and coordination notes as they work. CarsinOS Agent Mail remains the communication substrate and authority; a safe chatter producer supplies deliberately redacted working notes. Slack and the owner's Rust COLACK project are interaction/code references, not product names or competing message stores. Threading and typing indicators are optional capabilities and must not be fabricated when the substrate does not provide them. Anything needing the *user* still arrives as a ding on 4F.

### 2F — The Trenches
Direct involvement, on purpose — "where the boss gets out of the chair." Sub-rooms (tabs): **Boards** (full kanban parity with today: card editor, scripts, assets, run-card; boss drag/moves emit intent to ExecAss — nothing shuffles silently), **Calendar** (month/week, run-now/pause/resume, every date clickable), **Plan** (Strategy: goals/projects/tasks CRUD, insights/spend), **Teams / Staff Directory** (permanent staff only: ExecAss + wired-in model staff; each staff card contains its own live task-worker chips and an "open their memory" drill-in; org tree; Hire), **History & Receipts** (delegation history table, receipt chains, runbook explorer). This is an organizational view only; it introduces no payroll, salary, payment, or financial subsystem. Simple lock/reservation touch-points surface here; lock machinery lives in the Basement. Anything here can be **pinned to the Office** as a block.
...
4.5 **Office Chatter** — a read-first view over Agent Mail rooms, organized around workstreams. Agents post deliberately safe action notes and coordination; ExecAss replies; the boss may speak but never has to. Unreads are quiet dots, not badges screaming. Existing Agent Mail remains available as the full communication surface. COLACK may donate reusable Rust/display/configuration ideas, but is not a second authority or required label.
...
**Modules are data.** Floors and rooms declare themselves through registries rather than conditionals in the shell. A floor descriptor owns `{id, label, order, icon, shortcut, defaultRoomId, capabilityRequirements, visible}`; room descriptors own `{id, floorId, title, route, capabilityRequirements, blocks}`. Configuration may add, remove, hide, reorder, rename, or regroup floors and rooms. Unknown/disabled modules fail closed to a clear unavailable state, deep links resolve by stable IDs rather than display order, and the elevator renders the resolved registry. This is the successor to today's feature-toggle system and keeps future floors from becoming a shell rewrite.
...
- **P1 — Contract plumbing:** generated/hand-mapped TS types from `contracts/execass/v1/schema/`, schema-shaped fixtures, `execassApi` adapter (all 16 generated OpenAPI operations, idempotency + x-request-id + 409 reconcile), Tauri proof wrappers, one ExecAss store (summary/detail authoritative, WS events as invalidation, durable cursor resume). Mock-gateway ExecAss fixtures + WS invalidation behavior.
...
- **P3 — The Window:** Reef canvas on real presence (Dex-dependent; graceful placeholder until wired), Office Chatter over Agent Mail plus the safe working-note producer (Dex-dependent).
- **P4 — The Trenches:** remap boards/calendar/strategy/runbook/history into the floor with full capability parity; Teams / Staff Directory with nested task workers + per-staff memory drill-in; pin-to-Office.
